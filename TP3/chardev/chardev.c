 ...
 *  EXTRAIDO DE:  http://tldp.org/LDP/lkmpg/2.6/html/x569.html
 */
...
#define ENTERO_FIJO 0 //n° para encriptar-desencriptar
			   ...
			   loff_t * offset)
{
	...
	while (length && *msg_Ptr) {
		 ...
		 */
		put_user(*(msg_Ptr++), buffer++);//copies a single simple value from kernel space to user space. It supports simple types like char and int, but not larger data types like structures or arrays.
...
/*
	while (len && *msg_Ptr) {
		get_user(*(msg_Ptr++), buff++);//copies a single simple variable from user space to kernel space. It supports simple types like char and int, but not larger data types like structures or arrays.
		len--;
		bytes_write++;
	}*/
        ...
        for(bytes_write = 0; bytes_write < len && bytes_write < BUF_LEN; bytes_write++)
                get_user(msg[bytes_write], buff + bytes_write);
