...
#include <linux/slab.h>/* kmalloc() -> segun el libro aca esta definida kmalloc*/
...
ssize_t memoria_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos);
...
int memoria_open(struct inode *inode, struct file *filp) {
    ...
    //MOD_INC_USE_COUNT;
    try_module_get(THIS_MODULE);//incrementa el contador de usos
    /* Exito */
    return 0;
}
