...
char* formarString(const char cadena1[], const char cadena2[], const char cadena3[]);

int main(int argc, char* argv[]) {
    int nextOption;
    const char* const shortOptions = "stp:f:";
    const char* pid=NULL; //variable que guardara el id del proceso
    ...
    do{
        ...
        switch(nextOption){
            ...
            case 'p':
                pid=optarg;
             //   char ruta[] = *formarString("/proc/",pid, "/fd");
              //  char presentacion[] = *formarString("Los file descriptors accedidos por el proceso ",pid," son:");
                leerArchivo(formarString("/proc/",pid, "/fd"),formarString("Los file descriptors accedidos por el proceso ",pid," son:"));
                break;
            case 'f':
                pid=optarg;
                break;
...
char* formarString(const char cadena1[], const char cadena2[], const char cadena3[]){
    int tamanio;
    tamanio=strlen(cadena1)+strlen(cadena2)+strlen(cadena3);
    char resultado[tamanio];
